...
class X360:
    ...
    def udp_client_read_thread(self, selected_sock):
        global joystick_x
        global joystick_y
        global rnet_threads_running
        ...
        while True:
            try:
                ...
                twist = json.loads(data.decode())
                lin = twist['linear']['x']
                ang = twist['angular']['z']
                print(f"Received: linear={lin}, angular={ang}")
                if lin >  4: 
                    lin = 4
                elif  lin < - 4:
                    lin = -4 
                ...
                if ang > 0.8:
                    ...
                    ang = -0.8
                #v_x = lin/0.17 + 1.618*ang
                #v_y = lin/0.17 - 1.618*ang
                joystick_x = 0x100 + int(ang * 100.0 / 1.0)
                joystick_y = 0x100 + int(lin * 100.0 / 5.0)
                print('Send_udp:',joystick_x,joystick_y)
            except:
                print("Error reading USB: joystick")
                joystick_x = 0
                joystick_y = 0
...
if __name__ == "__main__":
    ...
    if usb_joystick_dev != '':
        ...
        udp_client_read_thread = threading.Thread(
            target=x360.udp_client_read_thread,
            args=(sock,),
            daemon=True)
        udp_client_read_thread.start()
